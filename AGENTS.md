...
Next.js 16.3.0 (App Router, Turbopack) + React 19 + Tailwind CSS v4 + shadcn/ui + next-intl. No unit tests; the only automated check is `pnpm seo:audit` (needs a running server). Public surface today: localized homepage (`/tr`, `/en`), a `/tools` landing page (indexed) with four noindex tool detail pages — one is functional (`CommitteeMinimumFinalForm`), the other three are stub shells — a full site header (nav, search, auth actions, mobile menu), locale switcher, breadcrumb, a site aside with a TUS exam countdown (homepage + tool pages), and complete SEO plumbing. `/courses`, `/library`, `/podcasts`, `/communities`, `/store`, `/login`, `/signup` are declared but have no pages yet. Audience is Turkish med students (TUS exam).
...
- `i18n/routing.ts` (`defineRouting`) already declares pathnames for `/courses`, `/library`, `/podcasts`, `/communities`, `/store`, `/login`, `/signup`, plus `/tools` and its four tool detail paths — but only `/tools` + `/` have pages; everything else 404s via `app/[locale]/[...rest]/page.tsx`. Root `app/page.tsx` only redirects `/` → `/tr`.
